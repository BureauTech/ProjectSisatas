...
const ViewRevisionsComponent = (props) => {
  ...
  const [rev_id, setRevId] = useState("");
  const [ata_id, setAtaId] = useState("");
  ...
  const body = {
    rev_id: revisoes.contemRevisoes.revId,
    ata_id: revisoes.contemRevisoes.ataId,
    usu_email: usuario.usuEmail,
  }


  const handleDelete = () => {
    revisaoServices.aprovarRevisao(body)
      .then((res) => {
        console.log(res)
